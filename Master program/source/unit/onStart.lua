...
MyCoroutines = {
    ...
    function()
        ...
        html_time = [[<div class="hud_time hud_container"><div style="text-align:center;">]] .. string.format("%02d/%02d/%04d %02d:%02d:%02d",day,month,year,hour,minute,second) .. [[</div><div style="text-align:right;"><hr class="hr-light"><style>.dm]] .. displayMode .. [[{border: 1px solid #28a745 !important;}</style><style>.dp]] .. displayType .. [[{border: 1px solid #28a745 !important;}</style><div>]]
    ...
    function()
        html_display_mode = '<style>.dm' .. displayMode .. '{background-color: #28a745 !important;}</style><div class="bg-light text-dark" style="position:absolute;bottom:70px;left:5px;padding:5px;font-size:' .. fontSize .. 'px;z-index:1000;font-weight:bold;">Alt + 9 - Display Mode: '
        for i,v in ipairs(displayModes) do
            html_display_mode = html_display_mode .. '<span class="dm' .. (i-1) .. '" style="margin:5px;">' .. v .. '</span>'
            if i < #displayModes then html_display_mode = html_display_mode .. "|" end
        end
        html_display_mode = html_display_mode .. '</div>'
    end,
    function()
